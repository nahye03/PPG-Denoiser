...
class TrainSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        LR_sr = np.array(Image.fromarray(LR_sr).resize((w, h), Image.BICUBIC))
        ### Ref and Ref_sr
        Ref_sub = imread(self.ref_list[idx])
        h2, w2 = Ref_sub.shape[:2]
        Ref_sub = np.array(Image.fromarray(Ref_sub).resize((w2, h2), Image.BICUBIC))
        ...
        80 -> input size
        '''
        Ref = np.zeros((80, 80, 3))
        Ref_sr = np.zeros((80, 80, 3))
        Ref[:h2, :w2, :] = Ref_sub
...
class TestSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h2, w2 = Ref.shape[:2]
        h2, w2 = h2//4*4, w2//4*4
        Ref = np.array(Image.fromarray(Ref).resize((w2, h2), Image.BICUBIC))
        Ref = Ref[:h2, :w2, :]
